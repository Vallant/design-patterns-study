 ...
 * @author stephan
 */
public class ActivityBarControllerCached implements ActivityBarController
{
  ...
  public ActivityBarControllerCached(ControllerCache cache)
  {
    this.cache = cache;
  }
  ...
  public void projectSelected(String project)
  {
    ...
    try
    {
      if(cache.contains(project.hashCode()))
        phases = cache.get(project.hashCode());
      else
      {
        phases = model.getProjectPhasesFor(project);
        cache.put(project.hashCode(), phases);
      }
